...
public class RobotScene implements Scene {
	...
	private static final long AC_ANIMATION_SPEED = TimeUtils
			.NanosecondsOfMilliseconds(1800);
	...
	private void paintLabels(Graphics2D g2d, int width, int height2,
			long elapsedTime) {
	...
	private void revalidateLabels(int width, int height, long elapsedTime) {
		...
		backedLabelRight
				.setPosition(width - _width - _x, height - _height - _y);
	}
	...
	private void paintActions(Graphics2D g2d, int width, int height,
			long elapsedTime) {
	...
	private void revalidateAction(Action ac, int width, int height) {
		...
		int _height = (int) (height * ((ac.getType() == ActionType.Attack) ? AC_ATK_HEIGHT
				: AC_DEF_HEIGHT));
	...
	private void calculateActionAnimation(int width, int height,
			long elapsedTime) {
		...
		if (acLeft != null) {
			...
			_x = (int) (leftRobo.getX() + leftRobo.getWidth()
					* (1 - DEF_OVERLAP));
			...
			if (!acLeft.getType().isDefendingType()) {
				...
				double possibleWidth = (rightRobo.getX() + rightRobo.getWidth()
						* ATK_OVERLAP)
						- _x - acLeft.getWidth();
				_x += (int) (acLeft.getDone() * possibleWidth);

				if (acLeft.getDone() >= 1
						&& acLeft.getDamage() == DamagePhase.Not) {
					acLeft.setVisible(false);

					if ((acRight == null || acRight.getType()
							.isDamageConsuming())) {
				...
				boolean isOtherDefendingOrNull = acRight == null
						|| acRight.getType().isDefendingType();
				boolean isAtkNear = acRight != null
						&& acRight.getDone() > NEAR_FOR_DEFENSE;
				boolean isAtkFinished = !isOtherDefendingOrNull
						&& acRight.getDone() >= 1;
			...
			acLeft.setTopLeftPosition(new Point(_x, acLeft.getTopLeftPosition()
					.getY()));
		}
		...
		if (acRight != null) {
			...
			if (!acRight.getType().isDefendingType()) {
				...
				double possibleWidth = _x
						- (leftRobo.getX() + leftRobo.getWidth()
								* (1 - ATK_OVERLAP));
				_x -= (int) (acRight.getDone() * possibleWidth);

				if (acRight.getDone() >= 1
						&& acRight.getDamage() == DamagePhase.Not) {
					acRight.setVisible(false);
				...
				boolean isOtherDefendingOrNull = acLeft == null
						|| acLeft.getType().isDefendingType();
				boolean isAtkNear = acLeft != null
						&& acLeft.getDone() > NEAR_FOR_DEFENSE;
				boolean isAtkFinished = !isOtherDefendingOrNull
						&& acLeft.getDone() >= 1;
			...
			acRight.setTopLeftPosition(new Point(_x, acRight
					.getTopLeftPosition().getY()));
		}
